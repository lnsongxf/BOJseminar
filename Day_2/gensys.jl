...
function gensys(Γ0, Γ1, c, Ψ, Π, args...)
    ...
    F = try
        ...
        if isa(ex, Schur.LAPACKException)
...
#     etawt = Ac_mul_B(qt2, Π) # deprecated
    etawt = qt2'*Π
    neta = size(Π, 2)
...
#         etawt1 = Ac_mul_B(qt1, Π) # deprecated
        etawt1 = qt1'*Π
...
#         loosesvd = svdfact!(loose)
    ...
    tmat = hcat(Matrix(1.0I,n-nunstab,n-nunstab), -(ueta * (deta \ veta') * veta1 * (deta1*ueta1'))')
...
#     impact = G0I * vcat(tmat * Ac_mul_B(qt, Ψ), zeros(nunstab, size(Ψ, 2)))
    ...
    loose = G0I * vcat(etawt1 * (Matrix(1.0I,neta,neta) - (veta*veta')), zeros(nunstab, neta))
